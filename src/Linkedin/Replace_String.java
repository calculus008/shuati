...
public class Replace_String {
    /**
     * 字符串替换就是给你一个原字符串s，一个目标串p，一个替换字符串t，把s里面所有的p替换成t，
     * 比如给s="abcdefg", p="bc",t="x"，返回"axdefg"
     *
     * 面试官期望的是O(n)的解法，比如用一个sliding window记录当前窗口每个字符出现频率，
     * 只有出现频率符合目标串p里面字符频率才进行字符串比较，我说如果遇见"aaaaaa",
    ...
    String replaceString(String original, String from, String to) {
        ...
        while (right < original.length()) {
            if (right - left < from.length()) {
                ...
                /**
                 * isSame only tells if number of each char in from is the same
                 * as in to. We still need to compare from and to to make if they
                 * are indeed the same.
                 */
                if (isSame && original.substring(left, right).equals(from)) { // 比较顺序
                    ...
                    /**
                     * move sliding window by jumping after the replaced substring
                     */
                    left = left + to.length();
                    right = left;
                    continue;
                ...
                /**
                 * here current window substring is not the same as from,
                 * we need to move the sliding window by more char -
                 * remove the left char and add the right char, then move
                 * left by 1.
                 */
