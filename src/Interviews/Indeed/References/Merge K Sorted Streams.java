...
一是内存消耗太大，二是不是in order的排序，你弄完了result还得sort一下这个算法不是最优。
...
<B> 输入是n个data stream 和 一个常数K，返回至少出现K次的数的list, 这个数必须在多于1个Stream中出现过。
    ...
    Stream有两个操作：
    interface Stream{
        boolean move(); //true means next position has value
        int getValue();
    }
    Ex: k = 2;
...
<G> 第一轮：get K appearances numbers in N sorted streams，问了复杂度，要求写几个test case跑一跑。
笔记里面的11和3都是一道题。
