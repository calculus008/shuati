 ...
 */
public class LE_901_Online_Stock_Span {
    /**
     ...
     * use stack, keeping "eating" previous one if it is smaller than current one.
     *
     * Time and Space : O(n)
     */
    class StockSpanner {
        class Element {
            int price, span;
            public Element(int price, int span) {
                this.price = price;
                this.span = span;
            }
        }
        ...
        public int next(int price) {
            if (stack.isEmpty()) {
                stack.push(new Element(price, 1));
                return 1;
            }
