...
public class LE_2090_K_Radius_Subarray_Averages {
    /**
    ...
    class Solution {
        public int[] getAverages(int[] nums, int k) {
            ...
            long size = 2 * k + 1; //also should be long in order to do division with sum

            for (int i = 0, j = 0, l = k; i < n; i++) { //!!! "i < n": even though value after idx i - k is all -1, the sum should still go on to the end of the array
                sum += nums[i];
