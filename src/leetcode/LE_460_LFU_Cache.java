...
public class LE_460_LFU_Cache {
    ...
    class LFUCache_Exersize_1 {
        ...
        public void put(int key, int value) {
            if (capacity <= 0) {// cover corner case
                return;
            }
            ...
            if (valueMap.size() == capacity) {//eviction act
            ...
            valueMap.put(key, value); // add a new key-value pair
            freqMap.put(key, 1); // we are adding a new key, so frequency is 1
            bucket.get(1).add(key);
            minFreq = 1;//!!!
        }

        private void update(int key) {
            int freq = freqMap.getOrDefault(key, 0);//#1.update freqMap
            int newFreq = freq + 1;
            freqMap.put(key, newFreq);

            if (!bucket.containsKey(newFreq)) { //#2.update bucket
            ...
            if (minFreq == freq && bucket.get(freq).size() == 0) {// #3.update minFreq !!!
                minFreq++;
            }
        }
    }
    ...
    class LFUCache {
        int capacity;
        int minFreq;
        Map<Integer, Integer> valueMap; // key value map
        Map<Integer, Integer> freqMap; //frequency of the key
        Map<Integer, LinkedHashSet<Integer>> bucket; // key - frequency, value - all keys in valueMap
                                                     // bucket entry is all the keys that have the same frequency
        ...
        public void put(int key, int value) {
            ...
            if (valueMap.size() >= capacity) {
                 ...
                 * Since this is the first element in this LinkedHashSet, remove is O(1)
                 */
                int evictKey = bucket.get(minFreq).iterator().next();
