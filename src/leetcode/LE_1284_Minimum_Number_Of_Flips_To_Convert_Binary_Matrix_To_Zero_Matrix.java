...
public class LE_1284_Minimum_Number_Of_Flips_To_Convert_Binary_Matrix_To_Zero_Matrix {
     ...
     *   a.当前状态是否在visited里
     *   b.当前状态是否是要求的最终状态
     * 3.所以注意到给定的条件：这是个binary matrix, 每个单元的值只会是0或者，而且"1 <= m, n <= 3"，也就是说最多会有9个值位0或1的单元。
     *   所以可以把一个这样的binary matrix的状态压缩到一个Integer里 (32bits),每个单元对应于整数中的一个bit. 例如：
     *   ｜0 1｜
     *   ｜1 0｜ => "0110" => 6
     *
     *   ｜0 0｜
     *   ｜0 0｜ => "0000" => 0
     *
     *   每次flip，只需要对相应的位和1做XOR:
     *    X ^ = (1 << i)
     *
     *   对状态的检查也可以变为对整数值的比较(O(1)).
     *
     * 4.BFS中的expand:
