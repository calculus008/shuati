...
public class LE_1618_Maximum_Font_To_Fit_A_Sentence_In_A_Screen {
    /**
     ...
     * Return the maximum font size you can use to display text on the screen. If text cannot fit on the display with any
     * font size, return -1.
     *
     * Example 1:
     ...
     * Binary Search
     *
     * It is simplified by assuming the screen contains only one line
     */
    interface FontInfo {
    ...
    class Solution {
        public int maxFont(String text, int w, int h, int[] fonts, FontInfo fontInfo) {
            ...
            while (l <= r) {
                ...
                if (canFit(text, w, h, fonts[m], fontInfo)) {
                    res = fonts[m];
                    l = m + 1;
                } else {
                    r = m - 1;
        ...
        private boolean canFit(String text, int w, int h, int fontSize, FontInfo fontInfo) {
            ...
            for (int i = 0; i < text.length(); i++) {
                ...
                int x = fontInfo.getWidth(fontSize, c);
                width += x;
