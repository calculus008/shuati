...
public class LE_1376_Time_Needed_To_Inform_All_Employees {
    /**
     ...
     * the i-th employee, manager[headID] = -1. Also, it is guaranteed that the subordination relationships have
     * a tree structure.
     *
     * The head of the company wants to inform all the company employees of an urgent piece of news. He will inform
     * his direct subordinates, and they will inform their subordinates, and so on until all employees know about the
     * urgent news.
     *
     ...
     * DFS
     *
     * Each employee, or a node, has a weight (informTime), this is to find the path with the max weight.
    ...
    class Solution1 {
        public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
            ...
            for (int i = 0; i < manager.length; i++) {
                map.computeIfAbsent(manager[i], l -> new ArrayList<>()).add(i);
            }

            return dfs(map, informTime, headID);
        }

        private int dfs(Map<Integer, List<Integer>> map, int[] informTime, int id) {
            if (!map.containsKey(id)) return 0;

            int max = 0;
            for (int n : map.get(id)) {
                max = Math.max(max, dfs(map, informTime, n));
            }
