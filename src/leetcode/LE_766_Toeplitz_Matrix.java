...
public class LE_766_Toeplitz_Matrix {
    /**
    ...
    class Solution_Mine {
        public boolean isToeplitzMatrix(int[][] matrix) {
            ...
            for (int i = 0; i < n; i++) {
                if (!check(matrix, 0, i, m, n)) return false;
            }

            for (int j = 1; j < m; j++) {
                if (!check(matrix, j, 0, m, n)) return false;
            }

            return true;
        }

        private boolean check(int[][] matrix, int x, int y, int m, int n) {
            int val = matrix[x][y];
            while (x + 1 < m && y + 1 < n) {
                x++;
                y++;
